...
/*
Name
  tadschest.cpp - guit3 Game Chest (favorites/library manager)
Function
  Ported from the legacy Win32 htmlt3 client's win32/w32chest.cpp, which
  this file otherwise tracks nearly verbatim (same classes, same dialog
  resources - the DLG_GAME_CHEST_ family and DLG_DOWNLOAD, in
  win32/htmlcmn.rc, pulled into guit3 via win32/htmlt3.rc's #include, and
  the same still-compiled CTadsDialog/CTadsDialogPropPage machinery in
  tadsdlg.cpp/tadsdlg2.cpp).
  Game Chest's own UI was never ImGui-native to begin with - it renders as
  an HTML page in the game's own text window via the ordinary HTML
  formatter/parser, and its handful of editor/wizard/progress dialogs are
  native, self-pumping Win32 modals exactly like the Options dialog's
  still-native Safety/NetSafety tabs (see the comment atop
  CHtmlPreferences::opt_render_gamechest_tab() in htmlpref.cpp) - so they
  keep working unmodified when invoked from an ImGui menu/link-click
  handler.  See imgui/migration.md, section 5.4 item O, for the port
  writeup.

  Two dependencies from the original file no longer exist in this port and
  are replaced here:
    - tadsreg.h/CTadsRegistry (HKEY_CURRENT_USER access) -> tadssettings.h/
      CTadsSettings, in CDlgWithProfilePopup::init_profile_popup() - this
      mirrors the exact enumeration pattern htmlpref.cpp's own
      opt_refresh_profile_list() already uses for the same
      "...\Profiles" key.
    - The download dialog's post-download file-type/"NoOpen" lookup under
      HKEY_CLASSES_ROOT has no CTadsSettings equivalent (that class is
      HKEY_CURRENT_USER-only), so it's kept as a small direct Win32
      registry call rather than reviving tadsreg.cpp.
  foldsel.h/foldsel2.cpp (the native folder-browse dialog used by the
  find-and-add wizard's root-folder page) are reactivated into the build
  for this feature rather than replaced - same "self-pumping native modal,
  fine to call from an ImGui handler" reasoning above; CTadsFolderDialog
  (the ImGui folder picker) has no blocking entry point, so it can't be
  called synchronously from this dialog's WM_COMMAND handler the way
  CTadsDialogFolderSel2::run_select_folder() can.

Modified
  09/08/01 MJRoberts  - creation (win32/w32chest.cpp)
*/
...
protected:
                            ...
                            const char *cur_profile)
    {
        ...
        /*
         *   fill in the list with the available profile names - same
         *   "...\Profiles" key and enumeration shape as htmlpref.cpp's
        ...
        for (key_idx = 0 ; ; ++key_idx)
        {
            ...
            /* get the profile at this index from the settings store */
            if (CTadsSettings::enum_subkeys(key, key_idx, subkey,
                                            sizeof(subkey))
...
public:
    ...
    int do_notify(NMHDR *nm, int ctl)
    {
        ...
        case PSN_WIZFINISH:
            ...
            /*
             *   tell the wizard that finishing is okay - DWL_MSGRESULT is
             *   32-bit-only (never defined on x64); tadsdlg.cpp's own
             *   do_dialog_msg() already draws this same distinction, see
             *   migration.md 5.6
             */
#if defined(_M_IX86) && defined(_WIN32)
            SetWindowLong(handle_, DWL_MSGRESULT, FALSE);
#else
            SetWindowLong(handle_, DWLP_MSGRESULT, FALSE);
#endif
...
public:
    ...
    virtual void init()
    {
        ...
        /*
         *   Note: the original Win32 client also registered itself here via
         *   CTadsApp::add_modeless()/remove_modeless() (below, in destroy()),
         *   which fed a modeless-dialog list that let the main MSG pump route
         *   keyboard navigation (Tab/Enter/Escape) to whichever modeless
         *   dialogs were open via IsDialogMessage(). Those two methods were
         *   removed from CTadsApp as confirmed-dead code before this feature
         *   existed (migration.md 5.4/L) - guit3 has no such classic MSG pump
         *   to begin with (event_loop() drives everything through GLFW/
         *   ImGui), and this dialog's own window procedure already handles
         *   its own WM_COMMAND traffic directly, so there's nothing to wire
         *   this dialog into.
         */
        ...
        case IDOK:
            ...
            if (!cancel_)
            {
                ...
                default:
                    ...
                    {
                        /*
                         *   HKEY_CLASSES_ROOT lookup - CTadsSettings only
                         *   covers HKEY_CURRENT_USER (the app's own
                         *   settings root), so this one small, self-
                         *   contained file-association check stays a
                         *   direct Win32 registry call rather than reviving
                         *   tadsreg.cpp for it
                         */
                        HKEY suffix_key;

                        /* try opening the suffix class key */
                        if (RegOpenKeyEx(HKEY_CLASSES_ROOT, suffix, 0,
                                         KEY_READ, &suffix_key)
                            == ERROR_SUCCESS)
                        {
                            char val[128];
                            ...
                            /* get the default value, which gives the class name */
                            if (RegQueryValueEx(suffix_key, "", 0, &val_type,
                                                (BYTE *)val, &val_len)
                                == ERROR_SUCCESS
                                && val_type == REG_SZ)
                            {
                                ...
                                if (RegOpenKeyEx(HKEY_CLASSES_ROOT, val, 0,
                                                 KEY_READ, &class_key)
                                    == ERROR_SUCCESS)
                                {
                                    /*
                                     *   if that succeeded, make sure it
                                     *   doesn't have a "NoOpen" value
                                     */
                                    if (RegQueryValueEx(
                                        class_key, "NoOpen", 0, 0, 0, 0)
                                        != ERROR_SUCCESS)
                                    {
                                        /*
