 ...
 *   Michael J. Roberts.
 *
 *   guit3 fork of win32/mpegamp/getbits.cpp.  The ONLY difference from the
 *   shared Win32 version is that the two functions that touch the input file
 *   - get_input() and dummy_getinfo() - go through the portable TADS osfile
 *   API (osfrbc / osfseek) instead of ReadFile / SetFilePointer, so the
 *   digitized-MP3 path has no Win32 dependency.  CMpegAmp::in_file is still
 *   declared HANDLE (void*) in the shared mpegamp.h - guit3 stores an
 *   osfildef* there (see mpegamp_w32.h / mpegamp_w32.cpp) and this file
 *   casts it back.  Keep everything else in sync with
 *   win32/mpegamp/getbits.cpp.  See htmltads/imgui/migration.md section 5.4/I.
 */
#include <string.h>
#include <stdio.h>

/* TADS OS layer, for the portable file-I/O calls */
#include <os.h>
...
int CMpegAmp::dummy_getinfo(int n)
{
        ...
        /* skip the bytes (osfseek returns non-zero on error) */
        if (osfseek((osfildef *)in_file, n, OSFSK_CUR) != 0)
                return GETHDR_ERR;
...
int CMpegAmp::get_input(unsigned char *bp, unsigned int size)
{
        ...
        /*
         *   Read the bytes through the osfile layer.  osfrbc() returns the
         *   number of bytes actually read; a short count means we've hit the
         *   end of the stream (or a read error - osfrbc can't distinguish the
         *   two, so both map to GETHDR_EOF, which the caller handles
         *   gracefully).
         */
        actual = osfrbc((osfildef *)in_file, bp, size);
        if (actual != size)
                return GETHDR_EOF;
